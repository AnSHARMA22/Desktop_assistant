...
from assistant import  Ui_MainWindow
...
def pdf_reader():
    book = open('SAMPLE-converted.pdf','rb')  #Open the Book in Binary Format
    pdfReader = PyPDF2.PdfFileReader(book)
    pages = pdfReader.numPages #Extract the Number of Pages
    speak(f'Total Number of Pages in this book is {pages}')
    speak('enter the page number i have to read')
    pg = int(input('Please Enter the page Number : '))
    page = pdfReader.getPage(pg) #getting to the page (starts from 0)
...
def weather(): #for Weather, Location , Lattitude, Longitude, Humidity, temperature
    ...
    if data_json['cod'] == 200:
        ...
        speak(f'Current location is {city1}')
...
if __name__ == "__main__":  # main program
    ...
    while True:
        ...
        if "where am i" in query or "what is my location now" in query or "location" in query:
            ...
            t = remember.read()
            speak(f"yes you told me to remember :  {t}")
            remember.close()
            ...
            while (cap.isOpened()):
               ...
               speak("Alert ,  Camera disconnected")
